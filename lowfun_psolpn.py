from eva_func import Func, LowProb1Func
import numpy as np
...
def main(dim=20, error_rate=0.3):
    logger.info(f'start main dim:{dim} error_Rate:{error_rate}')

    f = LowProb1Func(dim=dim, error_rate=error_rate, row=int(1e4), prob_1_max=0.2)
    # print(f.a)
    # smalla, smallb, smallbf02 = f.get_some_sample(0, 0.25, row_max=10000)
    # biga, bigb, bigbf = f.get_some_sample(0.95, 1.1)

    # print('end')

    best_score = f.evaluate(f.x)
    # print(score)
    ...
    for step in range(1000):
        ...
        if ress[index] > best_res:
            ...
            best_res = ress[index]
        logger.info(f'step:{step} best:{best_res} target:{best_score}')
        if best_res > best_score:
            print('?')

        if best_res + error_rate > 0.99:
            real_score = f.evaluate(best_x)
            logger.info(f'finish usetime:{time.time() - s} evaluate:{evaluate_num} ')
...
if __name__ == '__main__':
    ...
    data = {}
    for dim in range(35, 40, 1):
        success = 0
        all_run = 3
        for i in range(all_run):
            if main(dim=dim, error_rate=0.45):
                success += 1
