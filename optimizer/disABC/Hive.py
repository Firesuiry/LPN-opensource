...
class BeeHive(object):
    ...
    def run(self):
        ...
        while itr < self.max_itrs:
            ...
            itr += 1
            print(self.best.fitness, self.best.vector)
            if self.calls_to_fitness > self.maxfes:
                print(f'reach max fes break')
                break
            elif 1e100 > self.maxfes > self.calls_to_fitness and itr == self.max_itrs:
                # print(f'prolong fe:{self.calls_to_fitness},maxfe:{self.maxfes}')
                itr -= 1
    ...
    def __init__(self,
        ...
        ----------
            ...
            :param int maxfes                 : max function evaluates
        """
        self.maxfes = maxfes
        # generates a seed for the random number generator
        if seed is None:
